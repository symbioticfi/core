...
contract CheckpointsTrace512Test is Test {
    ...
    function _assertLatestCheckpoint(bool exist, uint48 key, uint256[2] memory value) internal {
        (bool _exist, uint48 _key, uint256[2] memory _value) = _ckpts.latestCheckpoint();
        assertEq(_exist, exist);
        assertEq(_key, key);
        _assertEq512(_value, value);
    }

    // tests
    function testPush(uint48[] memory keys, uint256[2][] memory values, uint48 pastKey) public {
        vm.assume(values.length > 0 && values.length <= keys.length);
        _prepareKeys(keys, _KEY_MAX_GAP);

        // initial state
        assertEq(_ckpts.length(), 0);
        _assertEq512(_ckpts.latest(), [uint256(0), 0]);
        _assertLatestCheckpoint(false, 0, [uint256(0), 0]);

        uint256 duplicates = 0;
        for (uint256 i = 0; i < keys.length; ++i) {
            uint48 key = keys[i];
            uint256[2] memory value = values[i % values.length];
            if (i > 0 && key == keys[i - 1]) ++duplicates;

            // push
            (uint256[2] memory oldValue, uint256[2] memory newValue) = _ckpts.push(key, value);

            uint256[2] memory expectedOld = [uint256(0), 0];
            if (i > 0) {
                expectedOld = values[(i - 1) % values.length];
            }
            _assertEq512(oldValue, expectedOld);
            _assertEq512(newValue, value);

            // check length & latest
            assertEq(_ckpts.length(), i + 1 - duplicates);
            _assertEq512(_ckpts.latest(), value);
            _assertLatestCheckpoint(true, key, value);
        }
        ...
        uint256[2] memory expectedLastValue = values[(keys.length - 1) % values.length];
        _assertEq512(_ckpts.pop(), expectedLastValue);

        uint256[2] memory oldValue_ = _ckpts.latest();
        (uint256[2] memory oldValue, uint256[2] memory newValue) = _ckpts.push(keys[keys.length - 1], expectedLastValue);

        _assertEq512(oldValue, oldValue_);
        _assertEq512(newValue, expectedLastValue);
    }
    ...
    function testLookup(uint48[] memory keys, uint256[2][] memory values, uint48 lookup) public {
        vm.assume(values.length > 0 && values.length <= keys.length);
        _prepareKeys(keys, _KEY_MAX_GAP);
        ...
        uint256[2] memory upper = [uint256(0), 0];
        uint256[2] memory lower = [uint256(0), 0];
        uint48 lowerKey = type(uint48).max;
        for (uint256 i = 0; i < keys.length; ++i) {
            uint48 key = keys[i];
            uint256[2] memory value = values[i % values.length];
        ...
        _assertEq512(_ckpts.upperLookupRecent(lookup), upper);
    }
    ...
    function testUpperLookupRecentWithHint(
        ...
        _assertEq512(resultWithHint, resultWithoutHint);
    }

    // Test upperLookupRecentCheckpoint without hint
    function testUpperLookupRecentCheckpoint(uint48[] memory keys, uint256[2][] memory values, uint48 lookup) public {
        vm.assume(values.length > 0 && values.length <= keys.length);
        _prepareKeys(keys, _KEY_MAX_GAP);
        ...
        uint48 expectedKey = 0;
        uint256[2] memory expectedValue = [uint256(0), 0];
        uint32 expectedIndex = 0;
        for (uint32 i = 0; i < _ckpts.length(); ++i) {
            Checkpoints.Checkpoint512 memory checkpoint = _ckpts.at(i);
            if (checkpoint._key <= lookup) {
                expectedExists = true;
                expectedKey = checkpoint._key;
                expectedValue = checkpoint._value;
                expectedIndex = i;
            } else {
                break;
            }
        ...
        // Test function
        (bool exists, uint48 key, uint256[2] memory value, uint32 index) = _ckpts.upperLookupRecentCheckpoint(lookup);
        assertEq(exists, expectedExists);
        if (exists) {
            assertEq(key, expectedKey);
            _assertEq512(value, expectedValue);
            assertEq(index, expectedIndex);
        }
    }
    ...
    function testUpperLookupRecentCheckpointWithHint(
        ...
        (bool existsWithHint, uint48 keyWithHint, uint256[2] memory valueWithHint, uint32 indexWithHint) =
            _ckpts.upperLookupRecentCheckpoint(lookup, hint);
        (bool existsWithoutHint, uint48 keyWithoutHint, uint256[2] memory valueWithoutHint, uint32 indexWithoutHint) =
            _ckpts.upperLookupRecentCheckpoint(lookup);

        assertEq(existsWithHint, existsWithoutHint);
        if (existsWithHint) {
            assertEq(keyWithHint, keyWithoutHint);
            _assertEq512(valueWithHint, valueWithoutHint);
            assertEq(indexWithHint, indexWithoutHint);
        }
    }

    // Test latest
    function testLatest(uint48[] memory keys, uint256[2][] memory values) public {
        vm.assume(values.length > 0 && values.length <= keys.length);
        _prepareKeys(keys, _KEY_MAX_GAP);

        uint256[2] memory expectedLatest = [uint256(0), 0];

        for (uint256 i = 0; i < keys.length; ++i) {
            _ckpts.push(keys[i], values[i % values.length]);
            expectedLatest = values[i % values.length];
            _assertEq512(_ckpts.latest(), expectedLatest);
        }
    }

    // Test latestCheckpoint
    function testLatestCheckpoint(uint48[] memory keys, uint256[2][] memory values) public {
        vm.assume(values.length > 0 && values.length <= keys.length);
        _prepareKeys(keys, _KEY_MAX_GAP);

        for (uint256 i = 0; i < keys.length; ++i) {
            uint48 expectedKey = keys[i];
            uint256[2] memory expectedValue = values[i % values.length];
            _ckpts.push(expectedKey, expectedValue);

            (bool exists, uint48 key, uint256[2] memory value) = _ckpts.latestCheckpoint();
            assertTrue(exists);
            assertEq(key, expectedKey);
            _assertEq512(value, expectedValue);
        }
    ...
    // Test length
    function testLength(uint48[] memory keys, uint256[2][] memory values) public {
        vm.assume(values.length > 0 && values.length <= keys.length);
        _prepareKeys(keys, _KEY_MAX_GAP);
    ...
    // Test at
    function testAt(uint48[] memory keys, uint256[2][] memory values, uint32 index) public {
        vm.assume(values.length > 0 && values.length <= keys.length);
        _prepareKeysUnrepeated(keys, _KEY_MAX_GAP);
        ...
        assertEq(checkpoint._key, keys[index]);
        _assertEq512(checkpoint._value, values[index % values.length]);
    }

    // Test pop
    function testPop(uint48[] memory keys, uint256[2][] memory values) public {
        vm.assume(values.length > 0 && values.length <= keys.length);
        _prepareKeys(keys, _KEY_MAX_GAP);
        ...
        uint256[2] memory lastValue = _ckpts.latest();
        uint256[2] memory poppedValue = _ckpts.pop();
        _assertEq512(poppedValue, lastValue);
        assertEq(_ckpts.length(), initialLength - 1);
    }
}
