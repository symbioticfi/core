...
interface IStakeForDelegator {
    function getAllocated(uint32 index, uint48 duration) external view returns (uint256);
}
...
abstract contract CoreV2StakeForInvariantHelper {
    ...
    function _assertStakeForSumLeCapacity(address vault_, address delegator_, uint32[] memory slots, uint48 duration)
        internal
        view
    {
