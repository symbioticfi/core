...
contract MockStakeForDelegator {
    mapping(uint96 => mapping(uint48 => uint256)) internal _allocated;

    function setAllocated(uint96 slot, uint48 duration, uint256 amount) external {
        _allocated[slot][duration] = amount;
    }

    function getAllocated(uint96 slot, uint48 duration) external view returns (uint256) {
