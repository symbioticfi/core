...
contract MainnetVaultMigrationForkTest is Test {
    ...
    function _assertMigratedDelegatorAndSlasher(
        ...
        IUniversalDelegator.Slot memory root = migratedDelegator.getSlot(0);
        IUniversalDelegator.Slot memory migratedSubvault = migratedDelegator.getSlot(MIGRATE_SUBVAULT_INDEX);
        assertEq(root.existChildren, 1, "unexpected root children");
        assertTrue(migratedSubvault.exists, "migrated subvault missing");
        assertTrue(migratedSubvault.isShared, "network restake migration should be shared");
        assertTrue(migratedSubvault.noAdapters, "migrated subvault should reserve liquid stake");
        assertEq(uint256(migratedSubvault.size), snapshot.totalStake, "migrated subvault size mismatch");
    }

    function _createLiveAllocationSlots(IUniversalDelegator migratedDelegator) internal {
        vm.startPrank(VAULT_OWNER);
        uint96 network1Slot =
            migratedDelegator.createSlot(NETWORK_1, MIGRATE_SUBVAULT_INDEX, false, false, uint128(NETWORK_1_SIZE));
        uint96 network2Slot =
            migratedDelegator.createSlot(NETWORK_2, MIGRATE_SUBVAULT_INDEX, false, false, uint128(NETWORK_2_SIZE));
        uint96 network3Slot =
            migratedDelegator.createSlot(NETWORK_3, MIGRATE_SUBVAULT_INDEX, false, false, uint128(NETWORK_3_SIZE));

        migratedDelegator.createSlot(
            _operatorKey(OPERATOR_1), network1Slot, false, false, uint128(NETWORK_1_OPERATOR_SIZE_AFTER_WITHDRAWALS)
        );
        migratedDelegator.createSlot(
            _operatorKey(OPERATOR_2), network1Slot, false, false, uint128(NETWORK_1_OPERATOR_SIZE_AFTER_WITHDRAWALS)
        );
        migratedDelegator.createSlot(
            _operatorKey(OPERATOR_1), network2Slot, false, false, uint128(NETWORK_2_OPERATOR_SIZE)
        );
        migratedDelegator.createSlot(
            _operatorKey(OPERATOR_2), network2Slot, false, false, uint128(NETWORK_2_OPERATOR_SIZE)
        );
        vm.stopPrank();

        assertEq(migratedDelegator.getSlotOfNetwork(NETWORK_1), network1Slot, "network1 slot mismatch");
        assertEq(migratedDelegator.getSlotOfNetwork(NETWORK_2), network2Slot, "network2 slot mismatch");
        assertEq(migratedDelegator.getSlotOfNetwork(NETWORK_3), network3Slot, "network3 slot mismatch");
        assertEq(migratedDelegator.getSlot(MIGRATE_SUBVAULT_INDEX).existChildren, 3, "network slot count mismatch");
    }
        ...
        view
    {
        ...
        assertEq(
            migratedDelegator.stake(NETWORK_1, OPERATOR_2),
            snapshot.network1Operator2Stake,
            "network1 operator2 stake mismatch"
        );
        assertEq(
        ...
        assertEq(migratedDelegator.maxNetworkLimit(NETWORK_1), type(uint208).max, "network1 max limit mismatch");
        assertEq(migratedDelegator.maxNetworkLimit(NETWORK_2), type(uint208).max, "network2 max limit mismatch");
        assertEq(migratedDelegator.maxNetworkLimit(NETWORK_3), type(uint208).max, "network3 max limit mismatch");
    }
