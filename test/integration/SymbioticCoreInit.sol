...
contract SymbioticCoreInit is SymbioticCoreInitBase, SymbioticInit, SymbioticCoreBindings {
    ...
    ) internal virtual override returns (Vm.Wallet memory) {
    ...
    function _stopBroadcastWhenCallerModeIsSingle(
    ...
    function _startBroadcastWhenCallerModeIsNotRecurrent(
        Vm.CallerMode callerMode,
        address deployer
    ) internal virtual override {
    ...
    function _stopBroadcastWhenCallerModeIsNotRecurrent(
        Vm.CallerMode callerMode
    ) internal virtual override {
        if (callerMode != VmSafe.CallerMode.RecurrentPrank) {
            vm.stopPrank();
    ...
    function _startBroadcastWhenCallerModeIsRecurrent(
        Vm.CallerMode callerMode,
        address deployer
    ) internal virtual override {
        if (callerMode == VmSafe.CallerMode.RecurrentPrank) {
    ...
    function _stopBroadcastWhenCallerModeIsSingleOrRecurrent(
        Vm.CallerMode callerMode
    ) internal virtual override {
