...
// forge script script/update/multisig/OptInVaultMultisig.s.sol:OptInVaultMultisigScript --rpc-url=RPC --private-key PRIVATE_KEY --broadcast
...
contract OptInVaultMultisigScript is OptInVaultBaseScript {
    ...
    function run() public {
        (bytes memory data, address target) = run(OPERATOR_VAULT_OPT_IN_SERVICE, VAULT, false);
        Logs.log(
            string.concat(
