...
// forge script script/update/multisig/SetResolverMultisig.s.sol:SetResolverMultisigScript --rpc-url=RPC -—sender SENDER_ADDRESS —-unlocked
...
contract SetResolverMultisigScript is SetResolverBaseScript {
    ...
    function run() public {
        (bytes memory data, address target) = run(VAULT, IDENTIFIER, RESOLVER);
        Logs.log(
            string.concat(
