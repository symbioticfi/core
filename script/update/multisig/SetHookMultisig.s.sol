...
// forge script script/update/multisig/SetHookMultisig.s.sol:SetHookMultisigScript --rpc-url=RPC --private-key PRIVATE_KEY --broadcast
...
contract SetHookMultisigScript is SetHookBaseScript {
    ...
    function run() public {
        (bytes memory data, address target) = run(VAULT, HOOK, false);
        Logs.log(
            string.concat(
